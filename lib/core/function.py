...
def train(config, train_loader, model, criterion, optimizer, epoch, output_dir, tb_log_dir, writer_dict):
    ...
    for i, (input, target, target_weight, meta) in enumerate(tqdm(train_loader, desc="train", leave=False)):
        # データの読み込み時間を計測
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            #       f'Time \t'\
            #       f'Loss {loss.val:.5f} ({loss.avg:.5f})\t' \
...
def validate(config, val_loader, val_dataset, model, criterion, output_dir, tb_log_dir, writer_dict=None):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(tqdm(val_loader, desc="valid", leave=False)):
            output = model(input)
    ...
    return perf_indicator, losses.avg, acc.avg
